...
#creating a new thread to make the flask interface and ros2 run in parallel
...
class LaunchRequestHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech_text = "The Robot is now activated"

        handler_input.response_builder.speak(speech_text).set_card(
            SimpleCard("Robot Activation", speech_text)).set_should_end_session(
            False)
...
skill_adapter = SkillAdapter(skill=skill_builder.create(), skill_id="amzn1.ask.skill.d722da99-32ae-4a85-a2cb-392b5bc781f3", app=app)
